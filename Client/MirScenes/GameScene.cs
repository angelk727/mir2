    ...
    public sealed class GameScene : MirScene
    {
        ...
        private void ObjectColourChanged(S.ObjectColourChanged p)
        {
            ...
            if (MapControl.Objects.TryGetValue(p.ObjectID, out var ob))
            {
                ...
                if (ob is MonsterObject monster)
                    monster.UpdateColourEffect(p.NameColour);
            }
        }
            ...
            if (MapControl.Objects.TryGetValue(p.ObjectID, out var ob))
            {
                ...
                switch (p.Effect)
                {
                    ...
                    case SpellEffect.Mon564NFlame:
                        ...
                        break;
                    case SpellEffect.Mon572BLightning:
                        ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.Mon572B], 406, 8, 300, ob) { Blend = true });
                        ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.Mon572B], 414, 10, 1000, ob) { Blend = true });
                        break;
                    ...
                    case SpellEffect.Mon580BLightning:
                        ...
                        break;
                    case SpellEffect.Mon621NSpikes:
                        ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.Mon621N], 580, 10, 300, ob) { Blend = false });
                        ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.Mon621N], 600, 10, 300, ob) { Blend = false });
                        break;
                    case SpellEffect.Mon622NSpikes:
                        ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.Mon622N], 570, 10, 300, ob) { Blend = false });
                        ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.Mon622N], 770, 10, 300, ob) { Blend = false });
                        break;
                }
        ...
        private void GuildNameRequest(S.GuildNameRequest p)
        {
            ...
            inputBox.InputTextBox.TextBox.KeyPress += (o, e) =>
            {
                ...
                {
                    ChatDialog.ReceiveChat("不能超过7个字符", ChatType.System);//待修复
                    e.Handled = true;
                }
