...
namespace Server.MirObjects
{
    public class TaoistHero : HeroObject
    {
        ...
        {
            if ((!CanCast || NextMagicSpell == Spell.None) && Owner.Info.HeroBehaviour == HeroBehaviour.反击)
            {
                MoveTo(Owner.Back);
                return;
            }
            ...
            if (!HasRangedSpell && InAttackRange() || NextMagicSpell == Spell.None && Owner.Info.HeroBehaviour == HeroBehaviour.攻击 && TargetDistance == 1)
            {
                Attack();
            ...
            if (!HasRangedSpell || NextMagicSpell == Spell.None && Owner.Info.HeroBehaviour == HeroBehaviour.攻击 && TargetDistance > 1)
