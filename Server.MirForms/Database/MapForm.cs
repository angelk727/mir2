        ...
        public static void Start(Envir envirToUpdate)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                if (lines[i].StartsWith("[")) // Read map info
                {
                    ...
                    newMapInfo.NoReincarnation = mapAttributes.Any(s => s.Equals("NOREINCARNATION", StringComparison.OrdinalIgnoreCase));
                    newMapInfo.GT = false;
                    newMapInfo.GTIndex = 0;
                    var gtAttr = mapAttributes.FirstOrDefault(s => s.StartsWith("GT(", StringComparison.OrdinalIgnoreCase));
                    if (gtAttr != null)
                    {
                        int l = gtAttr.IndexOf('(');
                        int r = gtAttr.IndexOf(')');
                        if (l >= 0 && r > l)
                        {
                            int gtIndex = int.Parse(gtAttr.Substring(l + 1, r - l - 1));
                            newMapInfo.GT = true;
                            newMapInfo.GTIndex = (byte)gtIndex;
                        }
                    }
                    ...
                    newMapInfo.NoHero = mapAttributes.Any(s => s.Equals("NOHERO", StringComparison.OrdinalIgnoreCase));
                    ...
                    newMapInfo.RequiredGroup = false;
                    newMapInfo.RequiredGroupSize = 0;

                    var reqAttr = mapAttributes.FirstOrDefault(
                        s => s.StartsWith("REQUIREDGROUP(", StringComparison.OrdinalIgnoreCase) ||
                             s.StartsWith("REQUIREGROUP(", StringComparison.OrdinalIgnoreCase));

                    if (reqAttr != null)
                    {
                        int l = reqAttr.IndexOf('(');
                        int r = reqAttr.IndexOf(')');
                        if (l >= 0 && r > l)
                        {
                            int groupSize = int.Parse(reqAttr.Substring(l + 1, r - l - 1));
                            newMapInfo.RequiredGroup = true;
                            newMapInfo.RequiredGroupSize = groupSize;
                        }
                    }
                    else
                    {
                        if (mapAttributes.Any(s => s.Equals("REQUIREDGROUP", StringComparison.OrdinalIgnoreCase) ||
                                                   s.Equals("REQUIREGROUP", StringComparison.OrdinalIgnoreCase)))
                        {
                            newMapInfo.RequiredGroup = true;
